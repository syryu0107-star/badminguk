...
test('planAutoAdvance: 전광판 예상 호출 시각 — 빈 코트 즉시(now·앞0)·대기 경기는 관측 페이스', () => {
  // 전광판(LiveScore)이 예정 경기에 붙이는 "약 HH:MM 호출 예상" 이 의존하는 계약:
  //  - 유휴 코트 맨 앞 예정 경기는 at≈now(곧 호출)·ahead 0
  //  - 같은 코트 뒤 경기는 앞 경기 수×페이스만큼 뒤로
  const matches = [
    { id: 'idle', status: 'scheduled', court_number: 2, match_number: 1, team1_entry_id: 'A', team2_entry_id: 'B' },
    { id: 'run', status: 'in_progress', court_number: 1, actual_start: new Date(NOW - 10 * 60000).toISOString(),
      team1_entry_id: 'C', team2_entry_id: 'D' },
    { id: 'wait', status: 'scheduled', court_number: 1, match_number: 1, team1_entry_id: 'E', team2_entry_id: 'F' },
  ]
  const obs = observedMatchMinutes(matches, { matchMinutes: 30, now: NOW }) // 진행 10분째 → 계획 하한 30
  const { estimates } = planAutoAdvance(matches, { matchMinutes: obs, now: NOW })
  assert.equal(estimates.idle.at, NOW)      // 빈 코트 → 지금 호출
  assert.equal(estimates.idle.ahead, 0)
  // 1번 코트 대기 경기: 진행 경기 종료 예상(시작+30분) 이후라 now 보다 뒤·앞 1경기
  assert.ok(estimates.wait.at > NOW)
  assert.equal(estimates.wait.ahead, 1)
})

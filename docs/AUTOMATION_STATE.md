...
## 실행 로그 (최신 위)
- 2026-07-14 · C1/심판 심판 코트 모드 선수 호출 확인 수신 — `src/pages/referee/CourtReferee.jsx`
  · **호출→응답 피드백 루프를 심판 쪽에서 닫음** — 직전 런이 `CourtReferee`에 선수 호출 버튼을 달았지만, 선수가 "가고 있어요"(`ackMatchCall`)를 눌러도 그 확인은 **주최자 `LiveDashboard`에만 "오는 중" 배지로** 떴다. 코트에서 직접 호출한 심판은 선수가 응답했는지 알 수 없어 무작정 기다려야 했다(near-zero touch 심판측 피드백 공백). **왜 non-human-gated·비파괴**: 확인 신호(`SIGNAL.CALL_ACK`)·구독 헬퍼(`subscribeCallAcks`)·발신부(선수 배너 `ackMatchCall`)가 이미 존재 — 심판 화면에서 **구독만 0**이던 순수 배선. 방송만(지속 저장·외부 발송 0)·새 스키마/외부 키/마이그레이션 0. **구현**: (1) `acked` 상태(matchId→확인 시각 ms). (2) `subscribeCallAcks(tournamentId)` useEffect 로 payload.matchId 확인 시 `setAcked`(LiveDashboard 와 동일 신호·수신 콜백 오류가 구독 죽이지 않음). (3) `handleCall`이 재호출 시 그 경기 ack 리셋(다시 응답할 때까지 배지 사라짐·LiveDashboard `ackTs≥calledAt` 가드와 동형). (4) CourtPanel 호출 버튼 아래 초록 "선수 확인 · 오는 중 (HH:MM)" 배지(`UserCheck`, `!live && !done && ackTs`). 주최자 구독은 그대로라 부전승 유예 관리 불변(추가 수신자일 뿐). 순수 배선(신규 순수 로직 없음)이라 엔진 회귀 유지. `npm test` **242/242**, `npx vite build` green. 배선 grep 확인. 다음 후보: LiveDashboard 노쇼 확인 대기(애매한 케이스) UX·접근성(aria)·심판 완료 화면 다음 경기 예상 시각. (자동화율 주최자 95%·선수 96%·심판 93→94%·운영 93% — 심판이 호출한 선수의 응답을 코트에서 바로 확인해 호출↔응답 루프가 심판 쪽에서도 닫힘)
  · **코트에 물리적으로 있는 심판이 대기 중인 현재 경기의 선수를 직접 호출(재호출)** — 직전 런이 QR 인쇄로 심판이 자기 코트 화면에 스스로 도달하는 경로를 완결한 뒤, 코드 실측으로 남은 심판 갭을 재선별: `CourtReferee`가 "다음 차례 경기"를 보여주지만 **선수를 코트로 부르는 트리거가 없었다**. 경기 호출은 오직 주최자 `LiveDashboard` 오케스트레이터만 발신 → **무인 진행이 꺼져 있거나 주최자가 대시보드를 안 열어두면 코트가 준비돼도 호출이 안 나가** 심판이 하염없이 기다려야 했다(near-zero touch 심판측 공백). 코트에 있는 심판이 "지금 이 코트 준비됐다"를 가장 잘 아는 사람이므로, 대기 중(경기 시작 전)인 현재 경기에 "선수 호출 (지금 코트로)" 버튼 신설 → 누르면 기존 `notify.callMatch`(인앱 실시간 방송 즉시 도달 + 지속 저장 best-effort)로 양 팀 선수에게 "지금 N번 코트로 입장하세요"가 나가고, 응답 없으면 "다시 호출"(재호출). **왜 non-human-gated·비파괴**: 기존 `callMatch` 그대로 재사용(로직 0 복제)·payload.entryIds 는 경기에서 파생돼 연결된 선수에게 방송으로 즉시 닿음(recipients profile id 는 지속 저장·오프라인 복구용)·새 스키마·외부 키·마이그레이션 0. 실시간 방송은 서버 웹푸시(VAPID·human-gated)와 무관. **구현**: (1) select 의 player 조인에 `id` 추가(recipients 수집용). (2) 순수 헬퍼 `callRecipients(match)`(LiveDashboard.recipientsOf 와 동형). (3) `handleCall`(useCallback·sending/호출시각 상태·양팀 미확정/전송중 가드·실패 시 상태 롤백). (4) CourtPanel 현재 경기 패널(`!live && !done && bothTeams`)에 파랑 아웃라인 호출 버튼(전송중/호출됨·HH:MM·다시 호출/기본 3상태). 주최자 자동 호출 경로 불변(추가 트리거일 뿐). QR/callMatch 순수 함수라 엔진 회귀 유지(신규 순수 로직 없음·UI 배선). `npm test` **242/242**, `npx vite build` green. 배선 grep 확인. 다음 후보: LiveDashboard 노쇼 확인 대기(애매한 케이스) UX·접근성(aria)·CourtView 현재 경기 대기 카드 예상 시각. (자동화율 주최자 95%·선수 96%·심판 92→93%·운영 93% — 심판이 코트에서 직접 선수를 부를 수 있어 주최자 대시보드 없이도 호출이 나가는 무인 호출 경로 이중화)
- 2026-07-14 · C1/심판 코트별 심판 QR 인쇄 — `src/pages/organizer/TournamentManage.jsx`
  · **심판이 자기 코트 점수판에 스스로 도달하는 마지막 경로(도달 경로) 완결 — 코트마다 QR 한 장을 뽑아 붙이면 심판이 찍는 순간 그 코트 경기 점수판이 열린다**. 직전 5런이 예측 엔진을 5개 표면(주최자·선수·전광판·심판·프로젝터)에 통일했고, 코드 실측으로 **가장 낮은 플로우(심판 91%)**의 실질 갭을 재선별 — `CourtReferee`(코트별 심판 모드) 화면은 이미 완성돼 있는데(현재/다음 경기 자동 배포·예상 시각·완료 후 다음 경기 원터치) **정작 심판이 그 화면에 닿는 방법**이 주최자가 자기 기기에서 여는 것뿐이었다(LiveDashboard `window.open`·TournamentManage 나열 링크). 코트마다 심판을 세우려면 주최자가 코트별 URL(`/referee/court/:id/:courtNo`)을 손으로 만들어 공유해야 해, DoD "심판은 이 화면만 열어두면 된다"가 **도달 경로에서 끊겨** 무인성이 깨졌다. **왜 non-human-gated·비파괴**: 순수 QR 엔진(`qrMatrix`/`qrSvgString`)과 인쇄(새 창→자동 print) 패턴이 이미 공개 전광판 공유용으로 존재 — 그대로 재사용, 새 라이브러리·서버·마이그레이션·외부 키 0. **구현**: (1) `refereeUrl(cn)` 헬퍼(코트 번호 있으면 per-court, 없으면 코트 선택 화면). (2) `openRefereePrintView` — `court_count`만큼 카드(큰 코트 번호+QR 300px+"이 QR을 찍으면 N번 코트 점수판이 열려요") 2열 그리드로 인쇄, 코트 수 미지정 시 안내. (3) TournamentManage "공개 스코어보드 공유" 아래에 "코트별 심판 QR" 카드 신설(인쇄 버튼+코트별 심판 모드 바로 열기+로그인 안내). QR/인쇄 순수 함수라 신규 테스트 불필요(엔진 회귀 유지). `npm test` **242/242**, `npx vite build` green(TournamentManage 청크). 배선 grep 확인(refereeUrl·openRefereePrintView 정의·onClick 참조). 다음 후보: LiveDashboard 노쇼 확인 대기(애매한 케이스) UX·CourtView `현재 경기` 대기(waiting) 카드도 예상 시각·접근성(aria). (자동화율 주최자 95%·선수 96%·심판 91→92%·운영 93% — 심판이 QR만 찍으면 자기 코트 무인 진행에 합류, 코트별 심판 배치의 마지막 수작업(URL 공유) 제거)
- 2026-07-14 · C6 코트 현황판(프로젝터) 예상 호출 시각 동적화 — `src/pages/organizer/CourtView.jsx`
  · **대회장 벽에 크게 띄우는 공개 코트 현황판만 남은 고정 계획 시각(scheduled_time)을 관측 페이스 예측으로 교체 — C6 예측 통일의 마지막 표면 완결(5개 표면 전부 동일 엔진)**. 직전 4런이 전광판(LiveScore)·심판 코트모드(CourtReferee)까지 동적 예상으로 통일했고, 원장 "다음 후보: CourtView 코트 현황판 예상 시각"을 코드 실측으로 확인 — `CourtView`의 "다음 경기" 라인(line 265)·"대기열" 행(line 691)이 여전히 `fmtTime(scheduled_time)`(고정 계획)이었다. **진단**: `planAutoAdvance`+`observedMatchMinutes` 관측 페이스 예측이 주최자 무인·선수 '내 경기'·공개 전광판·심판 코트모드 4표면엔 붙었는데, **정작 대회장에서 가장 크게·많은 사람이 보는 프로젝터 현황판만** 고정 계획 시각 — 대회가 부전승·빠른 경기로 앞서면 실제보다 늦은 시각을 보여 선수가 헛되이 늦게 오고(노쇼 위험), 밀리면 이른 시각을 보여 일찍 와 지침. **왜 non-human-gated·비파괴**: 순수 표시(DB 쓰기 0)·기존 엔진 재사용(로직 0 복제)·CourtView 쿼리가 이미 status·court_number·actual_start·scheduled_time·category_id·team1/2_entry_id 를 전부 select 해 추가 조회 0·마이그레이션/외부 키 0. **구현**: (1) `nowTick`(기존 무명 setTick→타임스탬프 상태)+30초 틱, `estimates` useMemo = `planAutoAdvance(matches,{matchMinutes:observedMatchMinutes(matches)})`.estimates(전 종목 큐 — 코트는 종목 넘나들며 순차 사용). (2) 순수 헬퍼 `estimateText(estimate,scheduledTime,now)` — at 있으면 "약 HH:MM 호출 예상"(now±60초는 "곧 호출 예상"), 없으면 계획 시각 "HH:MM 예정" 폴백. "다음 경기" 라인·대기열 행 두 곳의 `fmtTime` 교체(파랑 #7baef5 강조·"앞 N경기" 노출), 미배정/양팀 미확정이면 자연 폴백. 주최자·선수·전광판·심판·프로젝터 5개 표면이 같은 예측 엔진으로 완전 통일. 엔진은 이미 회귀 커버(planAutoAdvance/observedMatchMinutes)라 신규 테스트 불필요(UI 배선·순수 폴백 헬퍼만). `npm test` **242/242**, `npx vite build` green(CourtView 청크 확인). 배선 grep 확인(import·estimates useMemo·estimateText 3곳·estimates/now props). 다음 후보: 접근성(aria)·015/018 적용 후 실동작 검증·데모 워크스루·CourtView `현재 경기` 예정(waiting) 카드도 예상 시각. (자동화율 주최자 95%·선수 96%·심판 91%·운영 92→93% — 대회장에서 가장 많이 보이는 프로젝터 현황판까지 실제 페이스 예상 시각을 보게 돼 C6 예측이 전 표면 완전 통일, 헛걸음·노쇼 위험↓)
